...
import ProductSkeleton from "@/app/components/skeletons/ProductSkeletons"
...
interface ProductListProps {
  ...
  export default function CategeryProductList({ initialCategoryId }: ProductListProps) {
  ...
  useEffect(() => {
    if (initialCategoryId) {
      ...
      const selectedCategoryObj = categories.find((cat) => cat.id === Number(initialCategoryId))
      if (selectedCategoryObj) {
        setCategoryName(isRTL ? selectedCategoryObj.name_ar : selectedCategoryObj.name_en)
      }
    }
  }, [initialCategoryId, categories, isRTL])
  useEffect(() => {
    const fetchProducts = async () => {
      setLoading(true)
      setError(false)
      try {
        let url = `${BACKEND_BASE_URL}/product/api/products/`
        if (category) {
  ...
  return (
    <section id="product_listing" className={`pb-16 pt-30 bg-white mt-5 ${isRTL ? "rtl" : "ltr"}`}>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {categoryName && <h1 className="text-3xl font-bold mb-8">{categoryName}</h1>}
        {/* Filters and Search */}
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  if (selectedCategoryObj) {
                    setCategoryName(isRTL ? selectedCategoryObj.name_ar : selectedCategoryObj.name_en)
                    router.push(
                      `/category/${selectedCategory}?categoryName=${encodeURIComponent(isRTL ? selectedCategoryObj.name_ar : selectedCategoryObj.name_en)}`,
                    )
                  } else {
                    setCategoryName("")
